...
export function createPostgresGuardrailsRepository(db: PostgresDatabaseClient): GuardrailsRepository {
	...
	return {
		async listOwnedByWorkspace(workspaceId, userId, includeArchived = false) { return query(`${SELECT_DESIGNATED} WHERE (
			(g.workspace_id = $1 AND g.owner_user_id = $2)
			OR (g.is_account_default AND g.account_scope_key = (
				...
				FROM workspaces workspace WHERE workspace.id = $1 AND workspace.status = 'active'
			))
		)${includeArchived ? '' : " AND g.status = 'active'"} ORDER BY g.is_account_default DESC, g.updated_at DESC, g.id`, [workspaceId, userId]); },
		...
		getById,
		async getByIdInWorkspace(id, workspaceId) { return (await query<GuardrailWithVersionRow>(`${SELECT_DESIGNATED} WHERE g.id = $1 AND (
			g.workspace_id = $2 OR (g.is_account_default AND g.account_scope_key = (
				SELECT CASE workspace.scope_type
					WHEN 'personal' THEN 'personal:' || workspace.personal_owner_user_id
					WHEN 'organization' THEN 'organization:' || workspace.organization_id END
				FROM workspaces workspace WHERE workspace.id = $2 AND workspace.status = 'active'
			))
		)`, [id, workspaceId]))[0] ?? null; },
		...
		async listAssignments(guardrailId) { return query<GuardrailAssignmentRow>(`SELECT a.id, a.workspace_id, a.guardrail_id, a.scope_type, a.scope_id, a.created_by_user_id, a.management_source, a.assigned_by_user_id, a.created_at, g.name AS guardrail_name FROM guardrail_assignments a JOIN guardrails g ON g.id = a.guardrail_id WHERE a.guardrail_id = $1 ORDER BY a.scope_type, a.scope_id`, [guardrailId]); },
		async getEffectiveForRequest(workspaceId, userId, apiKeyId) { return query<EffectiveGuardrailRow>(`SELECT
			g.id, g.workspace_id, g.owner_user_id, g.name, g.description, g.status,
			g.is_workspace_default, g.is_account_default, g.account_scope_key,
			g.designated_version, g.latest_version, g.created_at, g.updated_at,
			v.id AS version_id, v.config_json AS version_config_json,
			v.created_by_user_id AS version_created_by_user_id, v.created_at AS version_created_at,
			CASE WHEN g.is_account_default THEN 'account-default:' || g.id
				WHEN g.is_workspace_default THEN 'workspace-default:' || g.id ELSE a.id END AS assignment_id,
			CASE WHEN g.is_account_default THEN 'account'
				WHEN g.is_workspace_default THEN 'workspace' ELSE a.scope_type END AS assignment_scope_type,
			CASE WHEN g.is_account_default THEN g.account_scope_key
				WHEN g.is_workspace_default THEN g.workspace_id ELSE a.scope_id END AS assignment_scope_id
			FROM workspaces request_workspace
			JOIN guardrails g ON g.status = 'active' AND (
				(g.is_account_default AND g.account_scope_key = CASE request_workspace.scope_type
			...
			LEFT JOIN guardrail_assignments a ON NOT g.is_workspace_default AND NOT g.is_account_default
		...
		async addVersion(params) {
			...
			await pg.begin(async (tx) => {
				...
				const updated = await tx.unsafe<Array<{ latest_version: number }>>(`UPDATE guardrails SET latest_version = latest_version + 1, designated_version = latest_version + 1, name = CASE WHEN is_workspace_default OR is_account_default THEN name ELSE $1 END, description = $2, updated_at = $3 WHERE id = $4 AND status = 'active'${protection} RETURNING latest_version`, [params.name, params.description, params.nowIso, params.guardrailId]);
		...
		async updateMetadata(id, patch) {
			...
			const defaultProtection = patch.name !== undefined || patch.status !== undefined
				? ' AND NOT is_workspace_default AND NOT is_account_default' : '';
			...
			if (!patch.preserveAdminManaged) {
				return Boolean((await query<{ id: string }>(`UPDATE guardrails SET name = CASE WHEN $1 THEN $2 ELSE name END, description = CASE WHEN $3 THEN $4 ELSE description END, status = CASE WHEN $5 THEN $6 ELSE status END, updated_at = $7 WHERE id = $8${defaultProtection} RETURNING id`, values))[0]);
			}
			let updated = false;
			await pg.begin(async (tx) => {
				const locked = await tx.unsafe<Array<{ id: string }>>('SELECT id FROM guardrails WHERE id = $1 FOR UPDATE', [id]);
				if (!locked[0]) return;
				updated = Boolean((await tx.unsafe<Array<{ id: string }>>(`UPDATE guardrails SET name = CASE WHEN $1 THEN $2 ELSE name END, description = CASE WHEN $3 THEN $4 ELSE description END, status = CASE WHEN $5 THEN $6 ELSE status END, updated_at = $7 WHERE id = $8${defaultProtection}${protection} RETURNING id`, values))[0]);
			});
			return updated;
		},
		...
		async upsertAssignment(params) {
			...
			await pg.begin(async (tx) => {
				const locked = await tx.unsafe<Array<{ id: string }>>(
					'SELECT id FROM guardrails WHERE id = $1 AND workspace_id = $2 AND NOT is_workspace_default AND NOT is_account_default FOR UPDATE',
