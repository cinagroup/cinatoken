-- 0068: private Batch API metadata and idempotent item ledger.
-- Request/response bodies are deliberately absent; they remain in private R2 objects.

CREATE TABLE batches (
  id TEXT PRIMARY KEY
    CHECK (length(id) BETWEEN 14 AND 128 AND substr(id, 1, 6) = 'batch_'),
  ...
  api_key_hash TEXT NOT NULL
    CHECK (length(api_key_hash) = 71 AND substr(api_key_hash, 1, 7) = 'sha256:'
      AND substr(api_key_hash, 8) NOT GLOB '*[^0-9a-f]*'),
  ...
  idempotency_key_hash TEXT
    CHECK (idempotency_key_hash IS NULL OR (
  ...
  CONSTRAINT batches_result_object_chk CHECK (
    ...
    OR (result_object_key IS NOT NULL AND length(result_object_key) BETWEEN 1 AND 1024
      AND instr(result_object_key, '..') = 0 AND result_sha256 IS NOT NULL)
  ),
...
CREATE TABLE batch_items (
  id TEXT NOT NULL UNIQUE
    CHECK (length(id) BETWEEN 18 AND 128 AND substr(id, 1, 10) = 'batch_req_'),
