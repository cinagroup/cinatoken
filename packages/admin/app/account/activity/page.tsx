...
export default function AccountActivityPage() {
		...
		void (async () => {
			try {
				const response = await fetch(
					`/api/user/activity?${buildQuery(range, status, apiKeyId, modelId, page)}`,
	...
	}, [apiKeyId, isSwitching, modelId, page, range, status, t, workspaceId]);

	const exportUrl = useMemo(
		() => `/api/user/activity/export.csv?${buildQuery(range, status, apiKeyId, modelId)}`,
		[apiKeyId, modelId, range, status],
	);
		...
		<div className="space-y-6">
			...
			<section className="console-panel rounded-xl border" style={{ borderColor: 'var(--console-border)' }}>
				<div className="border-b p-4" style={{ borderColor: 'var(--console-border)' }}>
					<form onSubmit={submitModel} className="grid gap-3 md:grid-cols-2 xl:grid-cols-[150px_170px_220px_minmax(220px,1fr)_auto]">
							...
							<tbody>
								{data.logs.map((row) => (
									<tr key={row.id} className="border-b last:border-b-0" style={{ borderColor: 'var(--console-border)' }}>
										...
										<td className="px-4 py-3"><code title={row.id} className="console-muted text-xs">{row.id.slice(0, 12)}</code></td>
