...
export default function AccountKeysPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const channelsData = await readPortalJson<{ channels: Channel[]; limits: ChannelLimits }>(channelsRes);
      ...
      if (channelsData?.success) {
        ...
        setForm((prev) => ({
          ...prev,
          channelType: prev.channelType || channelsData.data?.channels?.[0]?.channelType || '',
        }));
      }
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const data = await readPortalJson<SharedKeyRow & { apiKey: string; validation: string; validationReason: string | null }>(response);
      if (!response.ok || !data?.success) {
        setMessage({ kind: 'err', text: data?.message ?? t('keys.submitFailed') });
        return;
      }
        ...
        text:
          data.data?.validation === 'active'
            ...
            : data.data?.validation === 'invalid'
              ? t('keys.createdInvalid', { reason: data.data?.validationReason ?? '' })
  ...
  const patchKey = async (id: string, patch: Record<string, unknown>) => {
    ...
    if (!response.ok || !data?.success) {
      setMessage({ kind: 'err', text: data?.message ?? t('keys.updateFailed') });
      return;
    }
  ...
  const revalidate = async (id: string) => {
    const response = await fetch(`/api/user/shared-keys/${id}/revalidate`, { method: 'POST' });
    const data = await readPortalJson<SharedKeyRow>(response);
    if (!response.ok || !data?.success) {
      setMessage({ kind: 'err', text: data?.message ?? t('keys.updateFailed') });
      return;
  ...
  const removeKey = async (id: string) => {
    ...
    const response = await fetch(`/api/user/shared-keys/${id}`, { method: 'DELETE' });
    if (response.ok) {
    ...
    <div className="space-y-6">
      ...
      {message && (
        ...
        >
          {message.text}
          {revealedKey && (
            <div className="mt-2 break-all rounded bg-white/70 px-2 py-1 font-mono text-xs">{revealedKey}</div>
          )}
        </div>
      )}

      <form onSubmit={submit} className="space-y-4 rounded-lg border border-gray-200 bg-white p-4">
        <div className="text-sm font-medium text-gray-700">{t('keys.formTitle')}</div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <label className="space-y-1">
            <span className="text-xs text-gray-500">{t('keys.labelOptional')}</span>
            <input
              value={form.label}
          ...
          <label className="space-y-1">
            <span className="text-xs text-gray-500">{t('keys.inputPrice')}（$/1M）</span>
            <input
              type="number"
              min="0"
              step="0.0001"
              ...
              onChange={(event) => setForm((prev) => ({ ...prev, inputPrice: event.target.value }))}
              className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
              required
            />
          </label>
          <label className="space-y-1">
            <span className="text-xs text-gray-500">{t('keys.outputPrice')}（$/1M）</span>
            <input
              type="number"
              min="0"
              step="0.0001"
              ...
              onChange={(event) => setForm((prev) => ({ ...prev, outputPrice: event.target.value }))}
              className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
              required
            />
          </label>
          <label className="space-y-1">
            ...
            <input
              ...
              className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
            />
            <span className="text-[11px] text-gray-400">{t('keys.weightHint')}</span>
          </label>
        </div>
        {limits && (
      ...
      <div className="rounded-lg border border-gray-200 bg-white">
        {isLoading ? (
          ...
          <div className="px-4 py-8 text-center text-sm text-gray-500">{t('keys.empty')}</div>
        ) : (
                ...
                {rows.map((row) => (
                  <tr key={row.id} className="border-b border-gray-50 align-top last:border-0">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {row.failureReason && (
                        <div className="mt-1 max-w-48 text-[11px] text-red-400">{row.failureReason}</div>
                      )}
                    </td>
                    <td className="px-4 py-3 text-xs text-gray-600">
                      {t('keys.priceCell', { input: row.inputPrice, output: row.outputPrice })}
                    </td>
                    <td className="px-4 py-3 text-gray-700">{row.weight}</td>
                    <td className="px-4 py-3 text-xs text-gray-600">
                      <div>{t('keys.tokensCell', { input: (row.servedInputTokens ?? 0).toLocaleString(), output: (row.servedOutputTokens ?? 0).toLocaleString() })}</div>
                      <div className="text-gray-400">${(row.earnedTotal ?? 0).toFixed(4)}</div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-2 text-xs">
                        {row.status === 'active' && (
                          <button
                            type="button"
                            onClick={() => void patchKey(row.id, { status: 'paused' })}
