...
type Guardrail = { id: string; workspaceId: string; ownerUserId: string; name: string; description: string | null; status: 'active' | 'archived'; isWorkspaceDefault: boolean; isAccountDefault: boolean; accountScopeKey: string | null; designatedVersion: number; latestVersion: number; config: Record<string, unknown> | null; updatedAt: string };
...
export default function GuardrailManager({ mode }: { mode: 'user' | 'admin' }) {
	...
	const load = useCallback(async (signal?: AbortSignal) => {
		...
		try {
			...
			if (mode === 'user' && (responseWorkspaceId !== workspaceId
				|| nextRows.some((row) => !row.isAccountDefault && row.workspaceId !== workspaceId))) throw new Error(t('loadFailed'));
			setRows(nextRows);
			setMessage(null);
		} catch (error) {
	...
	const save = async () => {
		...
		try {
			...
			if (mode === 'user' && payload.data && !payload.data.isAccountDefault
				&& payload.data.workspaceId !== activeWorkspaceIdRef.current) return;
	...
	const edit = (row: Guardrail) => {
		if (mode === 'user' && !row.isAccountDefault && row.workspaceId !== workspaceId) return;
	...
	const patchRow = async (row: Guardrail, body: Record<string, unknown>) => {
		...
		else if (mode === 'user' && payload.data && !payload.data.isAccountDefault
			&& payload.data.workspaceId !== activeWorkspaceIdRef.current) return;
		else await load();
	};
	...
	const showDetails = async (row: Guardrail) => {
		...
		if (mode === 'user' && ((!row.isAccountDefault && row.workspaceId !== activeWorkspaceIdRef.current)
	...
	return <div className="space-y-6">
		...
		<EffectiveGuardrailPreview mode={mode} workspaceId={workspaceId} keys={keys} />
