...
export type ProviderRoutingResult =
	| { ok: true; body: Record<string, unknown>; routes: RouteResult[]; hasPreferences: boolean }
...
export type PreparedProviderRouting = {
	...
	apply(routes: RouteResult[]): ProviderRoutingResult;
};
...
const STRUCTURAL_REQUEST_KEYS = new Set([
	'model', 'models', 'messages', 'input', 'contents', 'prompt', 'stream',
...
export function prepareProviderRoutingPreferences(body: Record<string, unknown>):
	...
	if (!Object.prototype.hasOwnProperty.call(body, 'provider')) {
		return { ok: true, value: {
			body, hasPreferences: false, requireZdr: false, preferences: null,
			apply: (routes) => applyDefaultEndpointEligibility(body, routes),
		} };
	}
	const parsed = parseProviderPreferences(body.provider);
	...
	return { ok: true, value: {
		body: upstreamBody,
		hasPreferences: true,
		requireZdr: parsed.value.requireZdr,
		preferences: parsed.value,
		apply: (routes) => applyPreparedProviderRouting(parsed.value, upstreamBody, routes),
	} };
}

function routeMatchesSelector(route: RouteResult, selector: string): boolean {
	const normalized = selector.toLowerCase();
	const endpointSlug = route.endpoint?.selectorSlug;
	if (endpointSlug) {
		if (endpointSlug === normalized) return true;
		...
		if (variantSeparator > 0) {
			...
			if (route.endpoint?.endpointClass !== 'standard') return false;
...
function firstMatchingSelectorIndex(route: RouteResult, selectors: string[]): number {
	return selectors.findIndex((selector) => routeMatchesSelector(route, selector));
}
...
function hasExactEndpointSelector(route: RouteResult, selectors: readonly string[]): boolean {
	const slug = route.endpoint?.selectorSlug;
	return Boolean(slug && selectors.some((selector) => selector.toLowerCase() === slug));
}
...
function applyPreparedProviderRouting(preferences: ProviderPreferences, upstreamBody: Record<string, unknown>, routes: RouteResult[]): ProviderRoutingResult {
	...
	let eligible = routes.filter((route) => {
		...
		if (preferences.only && !preferences.only.some((selector) => routeMatchesSelector(route, selector))) return false;
		if (preferences.ignore.some((selector) => routeMatchesSelector(route, selector))) return false;
	...
	if (preferences.order.length > 0) {
		const explicitlyOrdered = eligible.filter((route) => firstMatchingSelectorIndex(route, preferences.order) >= 0);
		const remaining = eligible.filter((route) => firstMatchingSelectorIndex(route, preferences.order) < 0);
		eligible = preferences.allowFallbacks ? [...explicitlyOrdered, ...remaining] : explicitlyOrdered;
		eligible = eligible.map((route) => ({
			...route,
			routePriority: (() => {
				const selectorIndex = firstMatchingSelectorIndex(route, preferences.order);
	...
	return { ok: true, body: upstreamBody, routes: eligible, hasPreferences: true };
}

/** Apply validated provider selection controls and strip them before egress. */
export function applyProviderRoutingPreferences(body: Record<string, unknown>, routes: RouteResult[]): ProviderRoutingResult {
	const prepared = prepareProviderRoutingPreferences(body);
	if (!prepared.ok) return prepared;
	return prepared.value.apply(routes);
}
